...
class Network(nn.Module):
    def __init__(self, spec, stem_out_channels, num_stacks, num_modules_per_stack, num_labels):
        '''
        Args:
            spec: GenotypeSpec object containing the adjacency matrix and op labels for the model.
            stem_out_channels: number of output channels for the initial stem convolution.
            num_stacks: number of stacks in the model. Each stack is a group of modules
                that operate at the same feature map size. The first stack operates at the same
                feature map size as the input and each subsequent stack operates at half the
                feature map size of the previous stack.
            num_modules_per_stack: number of modules in each stack. Each module is a cell defined
                by the adjacency matrix and op labels in spec.
            num_labels: number of output labels for the model.
        '''
